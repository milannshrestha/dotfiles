...
const cmds = __importStar(__webpack_require__(11));
const ctx_1 = __webpack_require__(54);
const downloader_1 = __webpack_require__(12);
const status_display_1 = __webpack_require__(56);
async function activate(context) {
    ...
    const bin = client_1.resolveBin(ctx.config, serverRoot);
    if (!bin) {
        let msg = 'ra_lsp_server is not found, download from GitHub release?';
        const ret = await coc_nvim_1.workspace.showQuickpick(['Download', 'Cancel'], msg);
        if (ret === 1)
            return;
        try {
...
function resolveBin(config, serverRoot) {
    // 1. from config
    // 2. in PATH
    // 3. in coc-server-root
    let bin = expandPathResolving(config.raLspServerPath);
    if (bin === 'ra_lsp_server') {
        bin = which_1.default.sync(bin, { nothrow: true }) || path_1.join(serverRoot, bin);
    }
    if (!fs_1.existsSync(bin)) {
        return;
    }
    if (child_process_1.spawnSync(bin, ['--version'], { shell: process.platform === 'win32' }).status !== 0) {
        coc_nvim_1.workspace.showMessage(`Unable to execute '${bin} --version'`);
        return;
    }
    return bin;
}
exports.resolveBin = resolveBin;
function createClient(config, serverRoot) {
    var _a;
...
const coc_nvim_1 = __webpack_require__(1);
const downloader_1 = __webpack_require__(12);
const sourceChange = __importStar(__webpack_require__(19));
const analyzer_status_1 = __webpack_require__(46);
exports.analyzerStatus = analyzer_status_1.analyzerStatus;
const expand_macro_1 = __webpack_require__(47);
exports.expandMacro = expand_macro_1.expandMacro;
const join_lines_1 = __webpack_require__(48);
exports.joinLines = join_lines_1.joinLines;
const matching_brace_1 = __webpack_require__(49);
exports.matchingBrace = matching_brace_1.matchingBrace;
const on_enter_1 = __webpack_require__(50);
exports.onEnter = on_enter_1.onEnter;
const parent_module_1 = __webpack_require__(51);
exports.parentModule = parent_module_1.parentModule;
const runnables_1 = __webpack_require__(52);
exports.run = runnables_1.run;
exports.runSingle = runnables_1.runSingle;
const syntax_tree_1 = __webpack_require__(53);
exports.syntaxTree = syntax_tree_1.syntaxTree;
function collectGarbage(ctx) {
    return async () => {
        var _a;
        (_a = ctx.client) === null || _a === void 0 ? void 0 : _a.sendRequest('rust-analyzer/collectGarbage', null);
    };
}
...
function selectAndApplySourceChange() {
    return async (changes) => {
        var _a, _b;
        if (((_a = changes) === null || _a === void 0 ? void 0 : _a.length) === 1) {
            await sourceChange.applySourceChange(changes[0]);
        }
        else if (((_b = changes) === null || _b === void 0 ? void 0 : _b.length) > 0) {
...
function upgrade(ctx) {
    return async () => {
        await downloader_1.downloadServer(ctx.extCtx.storagePath);
    };
}
...
const coc_nvim_1 = __webpack_require__(1);
const fs_1 = __webpack_require__(2);
const node_fetch_1 = __importDefault(__webpack_require__(13));
...
async function downloadServer(root) {
    ...
    if (!latest) {
        statusItem.hide();
        coc_nvim_1.workspace.showMessage(`Can't get latest ra_lsp_server release`);
        return;
    }
    const _path = path_1.join(root, latest.name);
...
/* harmony import */ var https__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(17);
/* harmony import */ var zlib__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(18);
...
Object.defineProperty(exports, "__esModule", { value: true });
const coc_nvim_1 = __webpack_require__(1);
const vscode_languageserver_protocol_1 = __webpack_require__(20);
async function applySourceChange(change) {
    ...
    if (toOpen) {
        const toOpenUri = coc_nvim_1.Uri.parse(toOpen);
        await coc_nvim_1.workspace.readFile(toOpenUri.fsPath);
    }
    else if (toReveal) {
...
Object.defineProperty(exports, "__esModule", { value: true });
const vscode_jsonrpc_1 = __webpack_require__(21);
...
const callHierarchy = __webpack_require__(44);
...
const cancellation_1 = __webpack_require__(27);
exports.CancellationTokenSource = cancellation_1.CancellationTokenSource;
exports.CancellationToken = cancellation_1.CancellationToken;
const linkedMap_1 = __webpack_require__(28);
...
const net_1 = __webpack_require__(31);
const messageReader_1 = __webpack_require__(24);
const messageWriter_1 = __webpack_require__(26);
...
const protocol_workspaceFolders_1 = __webpack_require__(38);
...
function analyzerStatus(ctx) {
    return async () => {
        ...
        const ret = await ctx.client.sendRequest('rust-analyzer/analyzerStatus', null);
...
function expandMacro(ctx) {
    return async () => {
        ...
        const expanded = await ctx.client.sendRequest('rust-analyzer/expandMacro', param);
        if (!expanded) {
            return;
        }
...
Object.defineProperty(exports, "__esModule", { value: true });
const coc_nvim_1 = __webpack_require__(1);
const source_change_1 = __webpack_require__(19);
function joinLines(ctx) {
    return async () => {
        ...
        const change = await ctx.client.sendRequest('rust-analyzer/joinLines', param);
...
function matchingBrace(ctx) {
    return async () => {
        ...
        const response = await ctx.client.sendRequest('rust-analyzer/findMatchingBrace', request);
    ...
    return async () => {
        ...
        coc_nvim_1.events.on('TextChangedI', async (bufnr) => {
            ...
            if (lastChar && lastChar === '/' && Date.now() - lastTS < 40) {
                ...
                const change = await ctx.client.sendRequest('rust-analyzer/onEnter', request);
                if (!change) {
                    return;
                }
                await source_change_1.applySourceChange(change);
...
function parentModule(ctx) {
    return async () => {
        ...
        const response = await ctx.client.sendRequest('rust-analyzer/parentModule', param);
        if (response.length > 0) {
...
function run(ctx) {
    return async () => {
        ...
        const runnables = await ctx.client.sendRequest('rust-analyzer/runnables', params);
        const items = [];
...
function syntaxTree(ctx) {
    return async () => {
        ...
        const ret = await ctx.client.sendRequest('rust-analyzer/syntaxTree', param);
        await coc_nvim_1.workspace.nvim.command('tabnew').then(async () => {
            const buf = await coc_nvim_1.workspace.nvim.buffer;
...
class Ctx {
    ...
    async restartServer() {
        ...
        this.client = null;
        const client = client_1.createClient(this.config, this.extCtx.storagePath);
        if (!client) {
            return;
        }
...
class Config {
    constructor() {
        ...
        this.rainbowHighlightingOn = false;
        this.enableEnhancedTyping = true;
        this.raLspServerPath = RA_LSP_DEBUG || 'ra_lsp_server';
        this.lruCapacity = null;
        this.displayInlayHints = true;
        this.excludeGlobs = [];
        this.useClientWatching = true;
        this.featureFlags = {};
        this.cargoWatchOptions = {
            enable: true,
            arguments: [],
            command: '',
            allTargets: true
        ...
        this.prevEnhancedTyping = null;
        this.prevCargoFeatures = null;
        this.prevCargoWatchOptions = null;
        coc_nvim_1.workspace.onDidChangeConfiguration(() => this.userConfigChanged());
        this.userConfigChanged();
    }
    userConfigChanged() {
        const config = coc_nvim_1.workspace.getConfiguration('rust-analyzer');
        let requireReloadMessage = '';
        if (config.has('highlightingOn')) {
            this.highlightingOn = config.get('highlightingOn');
        }
        if (config.has('rainbowHighlightingOn')) {
            this.rainbowHighlightingOn = config.get('rainbowHighlightingOn');
        }
        // if (!this.highlightingOn && Server) {
        //   // Server.highlighter.removeHighlights();
        // }
        if (config.has('enableEnhancedTyping')) {
            this.enableEnhancedTyping = config.get('enableEnhancedTyping');
        ...
        if (config.has('raLspServerPath')) {
            this.raLspServerPath = RA_LSP_DEBUG || config.get('raLspServerPath');
        }
        if (config.has('cargo-watch.enable')) {
            this.cargoWatchOptions.enable = config.get('cargo-watch.enable', true);
        }
        if (config.has('cargo-watch.arguments')) {
            this.cargoWatchOptions.arguments = config.get('cargo-watch.arguments', []);
        }
        if (config.has('cargo-watch.command')) {
            this.cargoWatchOptions.command = config.get('cargo-watch.command', '');
        }
        if (config.has('cargo-watch.allTargets')) {
            this.cargoWatchOptions.allTargets = config.get('cargo-watch.allTargets', true);
        }
        if (config.has('lruCapacity')) {
            this.lruCapacity = config.get('lruCapacity');
        }
        if (config.has('displayInlayHints')) {
            this.displayInlayHints = config.get('displayInlayHints');
        }
        ...
        if (this.prevCargoFeatures !== null) {
            const changed = this.cargoFeatures.allFeatures !== this.prevCargoFeatures.allFeatures ||
        ...
        this.prevCargoFeatures = Object.assign({}, this.cargoFeatures);
        if (this.prevCargoWatchOptions !== null) {
            const changed = this.cargoWatchOptions.enable !== this.prevCargoWatchOptions.enable ||
        ...
        this.prevCargoWatchOptions = Object.assign({}, this.cargoWatchOptions);
        if (requireReloadMessage) {
            coc_nvim_1.workspace.showPrompt(`${requireReloadMessage}. Reload Now?`).then(prompt => {
                if (prompt) {
...
Object.defineProperty(exports, "__esModule", { value: true });
const coc_nvim_1 = __webpack_require__(1);
var WorkDoneProgressKind;
(function (WorkDoneProgressKind) {
    WorkDoneProgressKind["Begin"] = "begin";
    WorkDoneProgressKind["Report"] = "report";
    WorkDoneProgressKind["End"] = "end";
})(WorkDoneProgressKind || (WorkDoneProgressKind = {}));
class StatusDisplay {
    constructor(command) {
